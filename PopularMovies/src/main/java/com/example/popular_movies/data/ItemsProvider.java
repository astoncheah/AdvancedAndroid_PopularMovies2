...
public class ItemsProvider extends ContentProvider {
	...
	private static UriMatcher buildUriMatcher() {
		...
		matcher.addURI(authority, Tables.ITEMS, ITEMS);
		matcher.addURI(authority, Tables.ITEMS+"/#", ITEMS_ID);
		matcher.addURI(authority, Tables.FAVORITE+"/#",FILTER_FAVORITE);
		return matcher;
	}
	...
	public String getType(Uri uri) {
		...
		switch (match) {
			...
			case FILTER_FAVORITE:
				return ItemsContract.Items.CONTENT_FULTER_FAVORITE_TYPE;
			default:
	...
	private SelectionBuilder buildSelection(Uri uri, int match, SelectionBuilder builder) {
		...
		switch (match) {
			...
			case FILTER_FAVORITE: {
				final String val = paths.get(1);
				return builder.table(Tables.ITEMS).where(ItemsContract.Items.SAVE_AS_FAVORITE + "=?", val);
			}
			default: {
				throw new UnsupportedOperationException("Unknown uri: " + uri);
			}
